var $recaptchaV2SiteKey = '6LeBwQgrAAAAAJmADdrmY4QPxpFttA_AUAJby8hs';
...
sections['form-id-0'] = [
    ...
    ["street-address-from", "street-address-to", "zip-from", "zip-to"],
    ["no-of-pieces", "weight-lbs", "package-type", "vehicle"]
];
...
(function () {
    ...
    document.querySelectorAll("form.validate").forEach(function (form) {
        ...
        if (sections[form.id]) {
            ...
            var steps = form.querySelector('.steps');
            var stepsNextChild = steps.querySelector('.active')?.nextElementSibling;
            next.addEventListener('click', function (ev) {
                var errors = validate(form, fields) || {};
                nextSection(form, errors)
                if (stepsNextChild && stepsNextChild.tagName.toLowerCase() === 'div') {
                    stepsNextChild.classList.add('active');
                }
            })
        }
    });
